...
export class CronMissedDetector implements Detector {
  name = 'cron_missed_detector';

  private config: DetectorConfig;
  private scheduledTasks: Map<string, ScheduledTask> = new Map();
  private toleranceMs: number = 30000; // 30 seconds tolerance
  ...
  check(events: MonitoringEvent[]): DetectorResult {
    ...
    for (const [taskId, task] of this.scheduledTasks) {
      if (!task.expectedRunTime) continue;
      ...
      if (overdueMs > this.toleranceMs) {
        ...
        const taskRanRecently = recentEvents.some(
          e => e.type === 'cron_trigger' && e.metadata?.jobId === taskId
        );
