...
export class AnomalyDetector implements Detector {
  ...
  check(events: MonitoringEvent[]): DetectorResult {
    ...
    if (totalScore >= 5 || evasiveScore >= 3 || fabricationScore >= 3) {
      ...
      const issues: string[] = [];
      if (evasiveScore > 0) issues.push(`敷衍语气 (${evasiveScore}次)`);
      if (loopScore > 0) issues.push(`重复尝试 (${loopScore}次)`);
      if (fabricationScore > 0) issues.push(`未验证的完成声明 (${fabricationScore}次)`);
