...
import { OutputVerifierConfig, StrictnessLevel } from './types';

try { require('dotenv').config(); } catch {}

export function getDefaultConfig(): OutputVerifierConfig {
  return {
    enabled: true,
    strictness: 'standard',
    verifiers: {
...
export function loadConfig(overrides?: Partial<OutputVerifierConfig>): OutputVerifierConfig {
  ...
  if (process.env.OUTPUT_VERIFIER_STRICTNESS) {
    config.strictness = process.env.OUTPUT_VERIFIER_STRICTNESS as StrictnessLevel;
  }
