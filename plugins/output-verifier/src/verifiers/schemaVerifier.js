...
class SchemaVerifier {
    ...
    validateAgainstSchema(data, schema, path) {
        ...
        if (schema.properties && typeof data === 'object' && data !== null && !Array.isArray(data)) {
            ...
            for (const [key, propSchema] of Object.entries(properties)) {
                if (obj[key] !== undefined) {
                    const childErrors = this.validateAgainstSchema(obj[key], propSchema, path ? `${path}.${key}` : key);
        ...
        if (schema.enum && Array.isArray(schema.enum)) {
            if (!schema.enum.includes(data)) {
                errors.push({
                    path,
                    message: `Value at "${path || 'root'}" must be one of: ${schema.enum.join(', ')}`,
    ...
    buildResult(claimId, status, score, message, details, startTime, schemaErrors) {
        return {
            id: (0, uuid_1.v4)(),
            claimId,
