...
export class ApiVerifier implements Verifier {
  ...
  async verify(claim: AgentClaim): Promise<VerificationResult> {
    ...
    const apiCalls = (claim.toolCalls || []).filter(tc =>
      tc.tool.includes('api') ||
      tc.tool.includes('fetch') ||
      tc.tool.includes('http') ||
      tc.tool.includes('request') ||
    ...
    for (const call of apiCalls) {
      const url = call.args?.url as string || call.args?.endpoint as string;
      const method = (call.args?.method as string || 'GET').toUpperCase();
