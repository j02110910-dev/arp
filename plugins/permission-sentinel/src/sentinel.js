...
exports.PermissionSentinel = void 0;
...
class PermissionSentinel {
        ...
        if (this.config.blockedCommands) {
            for (const blocked of this.config.blockedCommands) {
                if (command.includes(blocked.toLowerCase())) {
                    return this.buildResult(action.id, 'critical', false, false, `Blocked: "${blocked}" is in the blacklist`, `blocked:${blocked}`);
                }
            }
        }
        // Check rules
        for (const rule of this.rules) {
            try {
                ...
                if (regex.test(action.command)) {
                    ...
                    return this.buildResult(action.id, rule.riskLevel, allowed, needsConfirm, rule.description, rule.id, rule.safeAlternative);
                }
            }
            catch {
                // Invalid regex, skip
     ...
     */
    restore(sanitized, originalValues) {
    ...
    buildResult(actionId, riskLevel, allowed, requiresConfirmation, reason, matchedRule, safeAlternative) {
        const result = {
            actionId,
            riskLevel,
            allowed,
...
exports.PermissionSentinel = PermissionSentinel;
